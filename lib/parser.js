...
class Parser {
  ...
  other(start) {
    ...
    let bracket = null
    let brackets = []
    // `decl()` strips a leading `*` or `_` IE hack prefix into `raws.before`,
    // so the flag must describe the prop after that strip too. Otherwise
    // prefixed custom properties like `*--x` are parsed as regular
    // declarations but stringified as custom properties, gaining a semicolon.
    let customProperty = /^[*_]?--/.test(start[1])
