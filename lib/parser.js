...
// `decl()` moves a single leading `*` or `_` (IE property hacks) from
// the property name to `raws.before`, so custom property detection has
// to look past the same prefix. Otherwise the parser and the stringifier
// disagree on whether a declaration is a custom property.
function isCustomProperty(name) {
  if (name[0] === '*' || name[0] === '_') name = name.slice(1)
...
class Parser {
  ...
  other(start) {
    ...
    let bracket = null
    let brackets = []
    let customProperty = isCustomProperty(start[1])
